...
def select_model(args, device):
    ...
    if model_id == 24:
        # RFDN baseline, AIM 2020 Efficient SR Challenge winner
        from models.team24_svan import SVAN
        name, data_range = f"{model_id:02}_SVAN", 255.0
        model_path = os.path.join('model_zoo', 'team24_svan.pth')
        model = SVAN(num_in_ch=3, num_out_ch=3, scale=4, num_feat=32, num_block=7, d_atten=64, conv_groups=2)
        model.load_state_dict(torch.load(model_path), strict=True)
